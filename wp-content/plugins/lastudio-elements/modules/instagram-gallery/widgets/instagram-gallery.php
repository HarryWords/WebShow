...
//		);
        ...
        if ( ! $this->get_access_token() ) {
            $this->add_control(
                ...
                array(
                    ...
                    'raw'  => sprintf(
                        esc_html__( 'Please set Instagram Access Token on the %1$s.', 'lastudio-elements' ),
                        '<a target="_blank" href="#">' . esc_html__( 'settings page', 'lastudio-elements' ) . '</a>'
                    ),
                    'condition' => array(
                        'endpoint' => 'self',
                    ),
        ...
        $this->add_control(
            'posts_counter',
            array(
                'label'   => esc_html__( 'Number of instagram posts', 'lastudio-elements' ),
        ...
        $this->add_control(
            'comments_icon',
            array(
                'label'       => esc_html__( 'Comments Icon', 'lastudio-elements' ),
        ...
        $this->add_control(
            'likes_icon',
            array(
                'label'       => esc_html__( 'Likes Icon', 'lastudio-elements' ),
                'type'        => Controls_Manager::ICON,
                'label_block' => true,
                'file'        => '',
                'default'     => 'fa fa-heart',
    ...
    public function render_gallery() {
        ...
        if ( 'self' === $settings['endpoint'] && ! $this->get_access_token() ) {
            return print esc_html__( 'Please, enter Access Token.', 'lastudio-elements' );
        }
        ...
        if ( ! empty( $posts ) ) {
            ...
            foreach ( $posts as $post_data ) {
                ...
                if ( $this->config['post_link'] ) {
                    ...
                    $link_format = apply_filters( 'lastudio-elements/instagram-gallery/link-format', $link_format );
                ...
                $html .= sprintf( '<div class="lastudio-instagram-gallery__item %s"><div class="lastudio-instagram-gallery__inner">%s</div></div>', $col_class, $item_html );
            }
    ...
    public function the_image( $item ) {
        ...
        $photo_format = apply_filters( 'lastudio-elements/instagram-gallery/photo-format', '<img class="lastudio-instagram-gallery__image la-lazyload-image" src="%1$s" data-src="%1$s" width="%2$s" height="%3$s" %4$s alt="">');

        $image = sprintf( $photo_format, esc_url( $post_photo_url ),  esc_url($width), esc_url($height), $srcset);
    ...
    public function the_caption( $item ) {
        ...
        $format = apply_filters(
            'lastudio-elements/instagram-gallery/the-caption-format', '<div class="lastudio-instagram-gallery__caption">%s</div>'
        );
    ...
    public function the_meta( $item ) {
        ...
        $format = apply_filters( 'lastudio-elements/instagram-gallery/the-meta-format', '<div class="lastudio-instagram-gallery__meta">%s</div>' );
    ...
    public function get_tags_url() {
        return apply_filters( 'lastudio-elements/instagram-gallery/get-tags-url', $this->api_url . 'explore/tags/%s/' );
    }
    ...
    public function get_self_url() {
        return apply_filters( 'lastudio-elements/instagram-gallery/get-self-url', $this->official_api_url . 'users/self/media/recent/' );
    }
    ...
    public function get_post_url() {
        return apply_filters( 'lastudio-elements/instagram-gallery/get-post-url', $this->api_url . 'p/%s/' );
    }
    ...
    public function _get_relation_photo_size( $photo_size ) {
        ...
        return apply_filters( 'lastudio-elements/instagram-gallery/relation-photo-size', $size, $photo_size );
    }
