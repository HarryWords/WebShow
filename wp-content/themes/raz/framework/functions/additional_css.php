...
$current_context = Raz()->get_current_context();


$page_title_bar_func = 'get';
if(Raz()->settings()->get_setting_by_context('page_title_bar_style', 'no') == 'yes'){
    $page_title_bar_func = 'get_setting_by_context';
}

$page_title_font_size = Raz()->settings()->get_setting_by_context('page_title_font_size', Raz()->settings()->get('page_title_font_size'));

if ( in_array('is_product', $current_context) ){
    if( 'on' == Raz()->settings()->get('single_product_override_page_title_bar', 'off') ) {
        $page_title_bar_func = 'get_setting_by_context';
    }
}
elseif ( in_array('is_single_la_portfolio', $current_context) ){
    if( 'on' == Raz()->settings()->get('single_portfolio_override_page_title_bar', 'off') ) {
        $page_title_bar_func = 'get_setting_by_context';
    }
}
elseif ( is_singular('post') ){
    if( 'on' == Raz()->settings()->get('single_post_override_page_title_bar', 'off') ) {
        $page_title_bar_func = 'get_setting_by_context';
    }
}
elseif ( in_array('is_shop', $current_context) || in_array('is_product_taxonomy', $current_context) ) {
    if( 'on' == Raz()->settings()->get('woo_override_page_title_bar', 'off') ) {
        $page_title_bar_func = 'get_setting_by_context';
    }
}
elseif ( in_array('is_archive_la_portfolio', $current_context) || in_array('is_tax_la_portfolio', $current_context) ) {
    if( 'on' == Raz()->settings()->get('archive_portfolio_override_page_title_bar', 'off') ) {
        $page_title_bar_func = 'get_setting_by_context';
    }
}


$page_title_bar_bg = Raz()->settings()->$page_title_bar_func('page_title_bar_background');
$page_title_bar_heading_color = Raz()->settings()->$page_title_bar_func('page_title_bar_heading_color', '#252634');
$page_title_bar_text_color = Raz()->settings()->$page_title_bar_func('page_title_bar_text_color', '#8a8a8a');
$page_title_bar_link_color = Raz()->settings()->$page_title_bar_func('page_title_bar_link_color', '#8a8a8a');
$page_title_bar_link_hover_color = Raz()->settings()->$page_title_bar_func('page_title_bar_link_hover_color', '#343538');
$page_title_bar_spacing = Raz()->settings()->$page_title_bar_func('page_title_bar_spacing');
$page_title_bar_spacing_desktop_small = Raz()->settings()->$page_title_bar_func('page_title_bar_spacing_desktop_small');
$page_title_bar_spacing_tablet = Raz()->settings()->$page_title_bar_func('page_title_bar_spacing_tablet');
$page_title_bar_spacing_mobile = Raz()->settings()->$page_title_bar_func('page_title_bar_spacing_mobile');
...
.section-page-header{
    ...
    <?php Raz_Helper::render_background_atts($page_title_bar_bg);?>
}
...
<?php
$main_space = Raz()->settings()->get_setting_by_context('main_space');
...
switch ($font_source) {
    ...
    $_s_highlight_font = (array) Raz()->settings()->get('highlight_font');
    ...
    $highlight_font_family = Raz()->settings()->get('highlight_google_font_face');
    break;

    case '3':
    $body_font_family = Raz()->settings()->get('main_typekit_font_face');
    $heading_font_family = Raz()->settings()->get('secondary_typekit_font_face');
    $highlight_font_family = Raz()->settings()->get('highlight_typekit_font_face');
    break;
}
...
$body_background = shortcode_atts(array(
    ...
    'size' => '',
    'color' => '#fff'
), Raz()->settings()->get('body_background'));
...
$header_background = shortcode_atts(array(
    ...
    'size' => '',
    'color' => '#fff'
), Raz()->settings()->get('header_background'));
...
$transparency_header_background = shortcode_atts(array(
    ...
    'size' => '',
    'color' => 'rgba(0,0,0,0)'
), Raz()->settings()->get('transparency_header_background'));
...
$footer_background = shortcode_atts(array(
    ...
    'size' => '',
    'color' => '#fff'
), Raz()->settings()->get('footer_background'));
...
$body_boxed_background = shortcode_atts(array(
    ...
    'size' => '',
    'color' => ''
), Raz()->settings()->get('body_boxed_background'));

$body_font_size = Raz()->settings()->get('body_font_size', 16);
$body_font_size = str_replace('px', '', $body_font_size);
$body_max_width = Raz()->settings()->get('body_max_width', 1230);
$body_max_width = str_replace('px', '', $body_max_width);
?>
body.raz-body{
    font-size: <?php echo esc_attr($body_font_size) ?>px;
    <?php Raz_Helper::render_background_atts($body_background);?>
}
body.raz-body.body-boxed #page.site{
    width: <?php echo esc_attr($body_max_width) ?>px;
    max-width: 100%;
    margin-left: auto;
    margin-right: auto;
    <?php Raz_Helper::render_background_atts($body_boxed_background);?>
}
body.raz-body.body-boxed .site-header .site-header-inner{
    max-width: <?php echo esc_attr($body_max_width) ?>px;
}
body.raz-body.body-boxed .site-header.is-sticky .site-header-inner{
    left: calc( (100% - <?php echo esc_attr($body_max_width) ?>px)/2 );
    left: -webkit-calc( (100% - <?php echo esc_attr($body_max_width) ?>px)/2 );
}
#masthead_aside,
.site-header .site-header-inner{
    <?php Raz_Helper::render_background_atts($header_background);?>
}
.enable-header-transparency .site-header:not(.is-sticky) .site-header-inner{
    <?php Raz_Helper::render_background_atts($transparency_header_background);?>
}

.footer-top{
    <?php Raz_Helper::render_background_atts($footer_background);?>
    <?php Raz_Helper::render_canvas_space(Raz()->settings()->get('footer_space'));?>
}
<?php
...
$popup_background =  shortcode_atts(array(
    ...
    'size' => '',
    'color' => ''
), Raz()->settings()->get('popup_background'));

?>
.open-newsletter-popup .lightcase-inlineWrap{
    <?php Raz_Helper::render_background_atts($popup_background);?>
}

<?php if( Raz()->settings()->get('catalog_mode', 'off') == 'on'){
...
<?php
$normal_header_height = str_replace('px', '', Raz()->settings()->get('header_height', 100));
$sticky_header_height = str_replace('px', '', Raz()->settings()->get('header_sticky_height', 80));

$header_sm_height = str_replace('px', '', Raz()->settings()->get('header_sm_height', 100));
$header_sm_sticky_height = str_replace('px', '', Raz()->settings()->get('header_sm_sticky_height', 80));

$header_mb_height = str_replace('px', '', Raz()->settings()->get('header_mb_height', 70));
$header_mb_sticky_height = str_replace('px', '', Raz()->settings()->get('header_mb_sticky_height', 70));


$megamenu_lv1 = str_replace('px', '', Raz()->settings()->get('megamenu_lv1', 16));
$megamenu_lv2 = str_replace('px', '', Raz()->settings()->get('megamenu_lv2', 12));
$megamenu_lv3 = str_replace('px', '', Raz()->settings()->get('megamenu_lv3', 12));
$megamenu_heading = str_replace('px', '', Raz()->settings()->get('megamenu_heading', 13));
...
<?php
$logo_height = str_replace('px', '', Raz()->settings()->get('logo_height', ''));
$logo_mobile_height = str_replace('px', '', Raz()->settings()->get('logo_mobile_height', ''));
?>
...
<?php endif;?>
<?php
$portfolio_thumbnail_height_mode = Raz()->settings()->get('portfolio_thumbnail_height_mode', 'original');
$portfolio_thumbnail_height_custom = Raz()->settings()->get('portfolio_thumbnail_height_custom', '70%');
...
$portfolio_item_space = intval(Raz()->settings()->get('portfolio_item_space', '0'));

?>
.la-portfolio-listing .lastudio-portfolio .loop__item.pf-custom-image-height .lastudio-portfolio__image{
    padding-bottom: <?php echo esc_attr($portfolio_thumbnail_height); ?>;
}
.la-portfolio-listing .lastudio-portfolio .lastudio-portfolio__list{
    margin-left: -<?php echo esc_attr($portfolio_item_space); ?>px;
    margin-right: -<?php echo esc_attr($portfolio_item_space); ?>px;
}
.la-portfolio-listing .lastudio-portfolio .loop__item{
    padding: <?php echo esc_attr($portfolio_item_space); ?>px;
}