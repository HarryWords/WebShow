...
class Raz_Blog {
    ...
    public function __construct(){
        ...
        add_filter( 'raz/setting/get_setting_by_context', array( $this, 'override_setting_by_context'), 20, 3);
    ...
    public function excerpt_length( $length ) {
        ...
        if ( ! is_null( Raz()->settings()->get( 'blog_excerpt_length' ) ) ) {
            $length = Raz()->settings()->get( 'blog_excerpt_length' );
        }
    ...
    public function override_setting_by_context( $value, $key, $context ){
        if($key == 'page_title_bar_layout'){
            if(in_array('is_home', $context) || in_array('is_category', $context) || in_array('is_tag', $context)){
                $from_blog_setting = Raz()->settings()->get('page_title_bar_layout_blog_global', 'off');
                $fn = 'get_term_meta';
                if(in_array('is_home', $context)){
                    $fn = 'get_post_meta';
                }
                $_from_current_setting = Raz()->settings()->$fn( get_queried_object_id(), $key );
    ...
    public function override_previous_next_post_format( $output, $format, $link, $post, $adjacent ){
        ...
        if(has_post_thumbnail($post)){
            $image = sprintf('<span class="nav_pnpp__image" style="background-image: url(%s);"></span>', get_the_post_thumbnail_url($post));
        }
