...
class Raz_Breadcrumbs {
    ...
    function __construct( $args = array() ) {
        ...
        $defaults = array(
            ...
            'labels'            => array(),
            'date_labels'       => array(),
            'action'            => 'raz/action/breadcrumbs/render_html',
            'css_namespace'     => array(),
            'path_type'         => 'default', // minified , full, default
            'post_taxonomy'     => apply_filters(
                'raz/filter/breadcrumbs/trail_taxonomies',
                array(
                    'post'      => 'category',
                    'product'	=> 'product_cat'
                )
        ...
        $this->args = apply_filters( 'raz/filter/breadcrumbs/args', wp_parse_args( $args, $defaults ) );
    ...
    public function get_trail() {
        ...
        $wrapper_classes = apply_filters( 'raz/filter/breadcrumbs/wrapper_classes', $wrapper_classes );
        ...
        do_action('raz/action/breadcrumbs/before_render_html', $this, $this->args);

        if ( true === $this->args['echo'] ) {
            echo raz_render_variable($result);
        }
        else {
            return $result;
        }
    ...
    public function get_items() {
        ...
        $breadcrumb = apply_filters( 'raz/filter/breadcrumbs/trail', $breadcrumb, $this->args );
    ...
    public function default_labels() {
        ...
        $labels = array(
            'browse'              => esc_html_x( 'Browse:', 'front-view', 'raz' ),
            'home'                => $this->home_title(),
            'error_404'           => esc_html_x( '404 Not Found', 'front-view', 'raz' ),
            'archives'            => esc_html_x( 'Archives', 'front-view', 'raz' ),
            'search'              => esc_html_x( 'Search results for &#8220;%s&#8221;', 'front-view', 'raz' ),
            'paged'               => esc_html_x( 'Page %s', 'front-view', 'raz' ),
            'archive_minute'      => esc_html_x( 'Minute %s', 'front-view', 'raz' ),
            'archive_week'        => esc_html_x( 'Week %s', 'front-view', 'raz' ),
    ...
    public function home_title() {
        ...
        $title            = esc_html_x( 'Home', 'front-view', 'raz' );
        $use_custom_title = apply_filters( 'raz/filter/breadcrumbs/custom_home_title', true );
    ...
    public function build_trail() {
        ...
        $custom_trail = apply_filters( 'raz/filter/breadcrumbs/custom_trail', false, $this->args );
         ...
         */
        $this->items = apply_filters( 'raz/filter/breadcrumbs/items', $this->items, $this->args, $this );
    ...
    public function add_blog_page() {
        ...
        if ( $page_for_posts_id ) {
            ...
            $page_title = esc_html__('Blog', 'raz');
            $url = esc_url(home_url('/'));
        }

        if ( is_paged() ) {
            $this->_add_item( 'link_format', $page_title, $url );
        }
        else{
            $this->_add_item( 'target_format', $page_title, '');
        }
    ...
    public function add_post_terms( $post_id ) {
        ...
        if ( ! empty( $this->args['post_taxonomy'][ $post_type ] ) ) {
            add_filter('get_object_terms', 'raz_exclude_demo_term_in_category');
            $post_terms = wp_get_post_terms( $post_id, $this->args['post_taxonomy'][ $post_type ] );
            raz_deactive_filter('get_object_terms', 'raz_exclude_demo_term_in_category');
    ...
    public function prepare_label( $label, $fallback = false ) {
        ...
        if ( false !== $fallback ) {
            $fallback = sprintf(
                apply_filters( 'raz/filter/breadcrumbs/icon_fallback', '<span class="hidden">%s</span>', $fallback ),
                esc_attr( $fallback )
            );
        }
        ...
        // Return default icon
        return sprintf(
            apply_filters( 'raz/filter/breadcrumbs/default_icon_format', '<span class="%1$s">%2$s</span>', $label, $fallback ),
    ...
    public function get_breadcrumb() {
        return apply_filters( 'raz/filter/breadcrumbs/get_breadcrumb', $this->crumbs, $this );
    }
}