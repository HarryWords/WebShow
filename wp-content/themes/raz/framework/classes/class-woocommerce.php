...
if(!class_exists('Raz_WooCommerce')) {

    class Raz_WooCommerce{
        ...
        public function __construct(){
            ...
            add_filter('raz/get_site_layout', array( $this, 'set_site_layout') );

            add_filter('raz/filter/sidebar_primary_name', array( $this, 'set_sidebar_for_shop'), 20 );
            ...
            // baseboot
            // add_action('raz/action/shop_loop_item_action_top', function(){ echo '<div class="wrap-addto">'; }, 5 );
            // add_action('raz/action/shop_loop_item_action_top', array( $this, 'add_quick_view_btn' ), 10 );
            // add_action('raz/action/shop_loop_item_action_top', array( $this, 'add_wishlist_btn' ), 20 );
            // add_action('raz/action/shop_loop_item_action_top', array( $this, 'add_compare_btn' ), 30 );
            // add_action('raz/action/shop_loop_item_action_top', array( $this, 'add_cart_btn' ), 40 );
            // add_action('raz/action/shop_loop_item_action_top', function(){ echo '</div>'; }, 50 );


            add_action('raz/action/add_count_up_timer_in_product_listing', array( $this, 'add_count_up_timer_in_product_listing' ), 1 );

            // add_action('raz/action/shop_loop_item_action', function(){ echo '<div class="wrap-addto">'; }, 5 );
            // add_action('raz/action/shop_loop_item_action', array( $this, 'add_quick_view_btn' ), 10 );
            // add_action('raz/action/shop_loop_item_action', array( $this, 'add_wishlist_btn' ), 30 );
            // add_action('raz/action/shop_loop_item_action', array( $this, 'add_compare_btn' ), 30 );
            // add_action('raz/action/shop_loop_item_action', array( $this, 'add_cart_btn' ), 40 );
            // add_action('raz/action/shop_loop_item_action', function(){ echo '</div>'; }, 50 );
             // baseboot end
            ...
            if( Raz()->settings()->get('product_single_hide_product_title', 'no') == 'yes'){
                remove_action('woocommerce_single_product_summary', 'woocommerce_template_single_title', 5);
            }

            add_filter('template_include', array( $this, 'load_quickview_template'), 20 );

            if( ! raz_string_to_bool(Raz()->settings()->get('move_woo_tabs_to_bottom', 'no')) ){
            ...
            if(Raz()->settings()->get('crosssell_products', 'off') == 'on'){
            ...
            if( Raz()->settings()->get('catalog_mode', 'off') == 'on'){
                // In Loop
                ...
                if( Raz()->settings()->get('catalog_mode_price', 'off') == 'on'){
            ...
            add_action( 'woocommerce_single_product_summary02', array( $this, 'my_custom_before_shop_loop_sidebar' ), 1);
        }


        // Add sidebar for shop loop header

        public function my_custom_before_shop_loop_sidebar() {
            if(is_active_sidebar('raz_widget_area_1')){
                dynamic_sidebar('raz_widget_area_1');
            }
        }
        ...
        public function wrapper_start(){
            ...
            echo '<div class="row">';
            echo '<main id="site-content" class="'. esc_attr(Raz()->layout()->get_main_content_css_class('col-xs-12 site-content')) .'">';
            echo '<div class="site-content-inner">';
            do_action( 'raz/action/before_render_main_inner' );
        ...
        public function wrapper_end(){
            ...
            do_action( 'raz/action/after_render_main_inner' );
            ...
            echo '</div><!-- ./container -->';
            echo '</div><!-- ./site-main -->';
            do_action( 'raz/action/after_render_main' );
        }
        ...
        public function set_sidebar_for_shop( $sidebar ) {
            ...
            if( in_array( 'is_woocommerce', $context ) ){
                ...
                if(in_array( 'is_archive', $context ) ){
                    ...
                    $sidebar = Raz()->settings()->get('shop_sidebar', $sidebar);

                    if(Raz()->settings()->get('shop_global_sidebar', false)){
                        /*
                    ...
                    if(in_array( 'is_shop', $context)){
                        if( ($single_sidebar = Raz()->settings()->get_post_meta( Raz()->get_page_id(), 'sidebar')) && !empty($single_sidebar) ){
                            $sidebar = $single_sidebar;
                        }
                    }
                    if(in_array( 'is_product_taxonomy', $context)){
                        if( ($tax_sidebar = Raz()->settings()->get_term_meta( get_queried_object_id(), 'sidebar')) && !empty($tax_sidebar) ){
                            $sidebar = $tax_sidebar;
                        }
                    }
                }
                ...
                elseif(in_array('is_product', $context)){
                    $sidebar = Raz()->settings()->get('products_sidebar', $sidebar);

                    if(Raz()->settings()->get('products_global_sidebar', false)){
                        /*
                         * Return global sidebar if option will be enable
                         * We don't need more checking in context
                         */
                    ...
                    if( ($single_sidebar = Raz()->settings()->get_post_meta( get_the_ID(), 'sidebar')) && !empty($single_sidebar) ){
                        $sidebar = $single_sidebar;
                    }
                }
            }
        ...
        public function add_class_to_product_loop($classes, $class, $post_id){
            ...
            if ( $product ) {
                ...
                if( 'on' == Raz()->settings()->get('woocommerce_enable_crossfade_effect') ){
                    $with_second_image = true;
                }

                $disable_second_image = raz_get_wc_loop_prop('disable_alt_image');
                ...
                if($with_second_image && (($galleries = $product->get_gallery_image_ids()) && !empty($galleries[0]))){
                    ...
                    $loop_layout = raz_get_wc_loop_prop('loop_layout');
                    $loop_style = raz_get_wc_loop_prop('loop_style');
                ...
                $enable_rating = Raz()->settings()->get('woocommerce_show_rating_on_catalog', 'off');
                if(get_option( 'woocommerce_enable_review_rating' ) === 'no'){
                    $enable_rating = 'off';
                }
                $classes[] = 'prod-rating-' . esc_attr(Raz()->settings()->get('woocommerce_show_rating_on_catalog', 'off'));

                if(raz_string_to_bool( $enable_rating )){
        ...
        public function add_script_resize_image_in_loop(){
            $image_size = raz_get_wc_loop_prop('image_size');
            if(!empty($image_size)) {
                Raz()->images()->before_resize();
            }
        }
        ...
        public function add_second_thumbnail_to_loop(){
            ...
            $prods_masonry = raz_get_wc_loop_prop('prods_masonry', false);
            ...
            $allow_caching = apply_filters('raz/help/allow_product_image_cache', false);
                ...
                if($prods_masonry){
                    if($image = wp_get_attachment_image_src($product->get_image_id(), $shop_catalog_size)){
                        ...
                        if($use_lazy_load){
                            ...
                            $style .= sprintf('background-image: url(%s);', esc_url(raz_get_relative_url($src)));
                        }
                        ...
                        if($prods_masonry){
                            ...
                            if( ($alt_image_src = wp_get_attachment_image_url($ids[0], $shop_catalog_size, false) ) && $alt_image_src ) {
                                ...
                                if($use_lazy_load){
                                    ...
                                    $style = sprintf('style="background-image: url(%s);"', raz_get_relative_url($alt_image_src));
                                }

                                $html_image_cache .= sprintf(
                                    '<div class="%s" data-background-image="%s"%s></div>',
                                    $elm_css_class,
                                    raz_get_relative_url($alt_image_src),
                                    $style
                                );
                            }
                        }
            ...
            echo raz_render_variable($html_image_cache);
        }
        ...
        public function add_quick_view_btn(){
            if( 'on' == Raz()->settings()->get('woocommerce_show_quickview_btn', 'off') ){
                global $product;
                printf(
                    ...
                    esc_attr_x('Quick Shop', 'front-view', 'raz'),
                    esc_attr_x('Quick Shop', 'front-view', 'raz')
                );
            }
        }
        ...
        public function add_cart_btn(){
            if( Raz()->settings()->get('catalog_mode', 'off') != 'on' && Raz()->settings()->get('woocommerce_show_addcart_btn', 'on') == 'on' ) {
        ...
        public function add_compare_btn(){
            ...
            if( Raz()->settings()->get('woocommerce_show_compare_btn', 'off') == 'on' ) {
                if ( !empty($yith_woocompare->obj) ) {
                    ...
                    printf(
                        ...
                        esc_attr($css_class),
                        esc_url($url),
                        esc_attr_x('Compare','front-view', 'raz'),
                    ...
                    $url = '#';
                    $text = esc_html_x('Compare','front-view', 'raz');
                    printf(
                        '<a class="%s" href="%s" title="%s" rel="nofollow" data-product_title="%s" data-product_id="%s"><span class="labtn-icon labtn-icon-compare"></span><span class="labtn-text">%s</span></a>',
                        esc_attr($css_class),
                        esc_url($url),
        ...
        public function add_wishlist_btn(){
            ...
            if(Raz()->settings()->get('woocommerce_show_wishlist_btn', 'off') == 'on'){
                global $product;
                if (function_exists('YITH_WCWL')) {
                    ...
                    if (YITH_WCWL()->is_product_in_wishlist($product->get_id(), $default_wishlist)) {
                        $text = esc_html_x('View Wishlist', 'front-view', 'raz');
                        $class = 'add_wishlist la-yith-wishlist button added';
                        $url = YITH_WCWL()->get_wishlist_url('');
                    }
                    else {
                        $text = esc_html_x('Add to Wishlist', 'front-view', 'raz');
                ...
                elseif(class_exists('TInvWL_Public_AddToWishlist')){
                    ...
                    if(isset($user_wishlist[0], $user_wishlist[0]['in']) && $user_wishlist[0]['in']){
                        ...
                        $url = '#';
                        $text = esc_html_x('Wishlist', 'front-view', 'raz');
                    }
                    printf(
                        '<a class="%s" href="%s" title="%s" rel="nofollow" data-product_title="%s" data-product_id="%s"><span class="labtn-icon labtn-icon-wishlist"></span><span class="labtn-text">%s</span></a>',
                        esc_attr($class),
                    ...
                    if(Raz_WooCommerce_Wishlist::is_product_in_wishlist($product->get_id())){
                        $class = 'add_wishlist button la-core-wishlist added';
                        $url = raz_get_wishlist_url();
                        $text = esc_html_x('View Wishlist', 'front-view', 'raz');
                    }
                    else{
                ...
                if(!empty($sale_price_dates_to)){
                    ?>
                        <div class="elementor-lastudio-countdown-timer lastudio-elements js-el" data-la_component="CountDownTimer">
                            ...
                            <div class="lastudio-countdown-timer__item-label"><?php esc_html_e('Days', 'raz') ?></div></div>
                            ...
                            <div class="lastudio-countdown-timer__item-label"><?php esc_html_e('Hours', 'raz');?></div></div>
                            ...
                            <div class="lastudio-countdown-timer__item-label"><?php esc_html_e('Mins', 'raz'); ?></div></div>
                            ...
                            <div class="lastudio-countdown-timer__item-label"><?php esc_html_e('Secs', 'raz'); ?></div></div>
                            </div>
                        </div>
                    <?php
                }
        ...
        public function shop_loop_item_excerpt(){
            $is_main_loop = raz_get_wc_loop_prop('is_main_loop', false);
            $loop_layout = raz_get_wc_loop_prop('loop_layout', 'grid');
            $loop_style = raz_get_wc_loop_prop('loop_style', '1');
            if( $is_main_loop || $loop_layout == 'list' || ( $loop_layout == 'grid' && $loop_style == 6 )  ) {
        ...
        public static function product_per_page(){
            return apply_filters('raz/filter/product_per_page', Raz()->settings()->get('product_per_page_default', 9));
        }

        public static function product_per_page_array(){
            $per_page_array = apply_filters('raz/filter/product_per_page_array', Raz()->settings()->get('product_per_page_allow', ''));
        ...
        public function set_cookie_default(){
            if (isset($_GET['per_page']) && $per_page = $_GET['per_page']) {
                add_filter('raz/filter/product_per_page', array( $this, 'get_parameter_per_page'));
            }
        }
        ...
        public function add_count_up_timer_to_single(){
            if(!isset($_GET['product_quickview']) && Raz()->settings()->get('show_product_countdown')){
                global $product;
                if($product->is_on_sale()){
        ...
        public function check_condition_show_upsell_crosssel(){
            if ( Raz()->settings()->get('related_products', 'off') != 'on' ) {
                remove_action('woocommerce_after_single_product_summary', 'woocommerce_output_related_products', 20);
            }
            if ( Raz()->settings()->get('upsell_products', 'off') != 'on' ) {
                remove_action('woocommerce_after_single_product_summary', 'woocommerce_upsell_display', 15);
            }
        }

        public function add_custom_tab($tabs){
            if ( Raz()->settings()->get('woo_enable_custom_tab', 'off') == 'on' ) {
                $tab_content = Raz()->settings()->get('woo_custom_tab_content', '');
                $tab_title = Raz()->settings()->get('woo_custom_tab_title', '');
                ...
                $tab_content_2 = Raz()->settings()->get('woo_custom_tab_content_2', '');
                $tab_title_2 = Raz()->settings()->get('woo_custom_tab_title_2', '');
                if(!empty($tab_title_2) && !empty($tab_content_2)){
                    $tabs['custom_tab_2'] = array(
                        'title' => $tab_title_2,
                        'priority' => 50,
                        'callback' => array( $this, 'get_custom_tab_content_2')
                    );
                }
                $tab_content_3 = Raz()->settings()->get('woo_custom_tab_content_3', '');
                $tab_title_3 = Raz()->settings()->get('woo_custom_tab_title_3', '');
                if(!empty($tab_title_3) && !empty($tab_content_3)){
                    $tabs['custom_tab_3'] = array(
                        'title' => $tab_title_3,
                        'priority' => 60,
                        'callback' => array( $this, 'get_custom_tab_content_3')
                    );
                }
            }
            return $tabs;
        ...
        public function get_custom_tab_content(){
            echo wp_kses_post(Raz_Helper::remove_js_autop( Raz()->settings()->get('woo_custom_tab_content', ''), true));
        }

        public function get_custom_tab_content_2(){
            echo wp_kses_post(Raz_Helper::remove_js_autop( Raz()->settings()->get('woo_custom_tab_content_2', ''), true));
        }

        public function get_custom_tab_content_3(){
            echo wp_kses_post(Raz_Helper::remove_js_autop( Raz()->settings()->get('woo_custom_tab_content_3', ''), true));
        }
        ...
        public function modify_availability_text_on_product_page( $availability, $product ){
            if ( ! $product->is_in_stock() ) {
                ...
                $availability = $availability = esc_html__( 'in stock', 'raz' );
            }
            return $availability;
        }
        ...
        public function add_sku_to_single_product(){
            ...
            if ( wc_product_sku_enabled() && ( $product->get_sku() || $product->is_type( 'variable' ) ) ){
                ...
                <div class="product_meta-top">
                    <span class="sku_wrapper"><span class="sku"><?php if($sku = $product->get_sku()) { echo raz_render_variable($sku); } else { esc_html__( 'N/A', 'raz' ); } ?></span></span>
                </div>
                <?php
            }
        }
        ...
        public function modify_ajax_cart_fragments( $fragments ){
            ...
            $text = '<span class="la-cart-text">'. esc_html_x('%s items','front-view', 'raz') .'</span>';
        ...
        public function add_coupon_form_into_cart(){
            if ( wc_coupons_enabled() ) : ?>
                <div class="la-coupon-form">
                    <h2><?php echo esc_html_x('Coupon Code', 'front-view', 'raz') ?></h2>
                    <p><?php echo esc_html_x('Enter your coupon code if you have one.','front-view', 'raz')?></p>
                    <div class="la-coupon">
                        <p class="form-row form-row-wide">
                            <input type="text" class="input-text" id="coupon_code_ref" value="" placeholder="<?php echo esc_attr_x( 'Enter your coupon code..', 'front-view', 'raz' ); ?>" />
                        </p>
                        <button type="button" class="button" id="coupon_btn_ref"><?php echo esc_html_x( 'Apply coupon', 'front-view', 'raz' ); ?></button>
                    </div>
                </div>
            <?php endif;
        }
        ...
        public function add_more_button_to_cart_from(){
            ...
            ?>
            <input type="submit" class="button btn-clear-cart" name="clear-cart" value="<?php echo esc_attr_x('Clear Cart', 'front-view', 'raz');?>">
            <?php
        }
        ...
        public function woocommerce_share(){
            if(Raz()->settings()->get('product_sharing') == 'on'){
                ...
                echo sprintf( '<label>%s</label>', esc_html_x('Share on', 'front-end', 'raz') );
                raz_social_sharing($post_link,$post_title,$image);
                echo '</div>';
            }
        }
        ...
        public function woocommerce_before_shipping_calculator(){
            printf(
                ...
                esc_html_x('Calculate Shipping', 'front-view', 'raz'),
                esc_html_x('Estimate your shipping fee *', 'front-view', 'raz')
            );
        }
