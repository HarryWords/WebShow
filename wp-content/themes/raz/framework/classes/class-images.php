...
class Raz_Images {
    ...
    public function image_downsize( $downsize = false , $id, $size ) {
        ...
        $crop = apply_filters('raz/filter/image_helper/crop', $crop, $id, $size);
    ...
    public function the_post_thumbnail($post = null, $size = 'thumbnail', $attr = '' ){
        echo raz_render_variable($this->get_post_thumbnail($post, $size, $attr));
    }
    ...
    public function render_image( $url = '', $attr = array()){
        if(empty($url)){
            ...
            if(Raz_Helper::is_enable_image_lazy()){
