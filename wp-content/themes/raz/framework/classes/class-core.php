...
class Raz {
    ...
    private static $original_option_name = 'raz_options';
    ...
    private function __construct() {
        ...
        wp_cache_add_non_persistent_groups( 'raz' );
        ...
        $this->settings       = new Raz_Setting();
        $this->images         = new Raz_Images();
        $this->layout         = new Raz_Layout();
        $this->options        = new Raz_Options();
        $this->favorite       = new Raz_FavoritePosts();

        new Raz_Init();
        new Raz_Blog();
        new Raz_Widget_CssClass();
    ...
    public function build_crumbs(){
        ...
        if( $this->settings()->get('google_rich_snippets', 'no') == 'yes' ){
            if(function_exists('wc') && is_object(wc()->structured_data)){
                add_action('raz/action/breadcrumbs/before_render_html', array(wc()->structured_data, 'generate_breadcrumblist_data'), 10);
            }
        }
        $this->breadcrumbs = new Raz_Breadcrumbs( $args );
    }

    public function set_is_updating() {
        if ( ! self::$is_updating && $_GET && isset( $_GET['raz_update'] ) && '1' == $_GET['raz_update'] ) {
    ...
    public function set_current_context(){
        ...
        $coreTypeAllows = array(
            ...
            'la_portfolio' => array(
                'single'    => 'is_single_la_portfolio',
                'archive'   => 'is_archive_la_portfolio',
                'tax'       => 'is_tax_la_portfolio',
                'taxonomy'  => array(
                    'la_portfolio_category' => 'is_tax_la_portfolio_category',
                    'la_portfolio_skill' => 'is_tax_la_portfolio_skill'
                )
            ),
        );
    ...
    public static function multilingual_options() {
        // Set the self::$lang.
        if ( ! in_array( Raz_Multilingual::get_active_language(), array( '', 'en', 'all' ) ) ) {
            self::$lang = '_' . Raz_Multilingual::get_active_language();
        }
