...
class Raz_Setting{
    ...
    public function __construct( $args = array() ) {
        $this->args = array_merge(array(
            'option_name'       => Raz::get_option_name(),
            'post_meta_name'    => Raz::get_original_option_name(),
            'term_meta_name'    => Raz::get_original_option_name()
        ), $args);
    }
    ...
    public function get( $key = '', $default = '' ){
        ...
        return apply_filters("raz/setting/option/get_single", $value, $key );
    }
    ...
    public function get_setting_by_context( $key, $default = '', $context = array()){
        ...
        if(in_array('is_home', $context)){
            ...
            if(!empty($_value)){
                if(is_array($_value)){
                    if(Raz_Helper::is_not_empty_array_ref($_value)){
                        $value = $_value;
                    }
                }else{
                    if($_value !== 'inherit'){
        ...
        elseif(in_array('is_archive', $context)){
            ...
            if(in_array('is_shop', $context)){
                ...
                if(Raz()->get_page_id()){
                    $_value = $this->get_post_meta( Raz()->get_page_id(), $key);
                    if(!empty($_value)){
                        if(is_array($_value)){
                            if(Raz_Helper::is_not_empty_array_ref($_value)){
                                $value = $_value;
            ...
            elseif(in_array('is_tax', $context) && is_tax(get_object_taxonomies( 'la_portfolio' ))){
                ...
                if($key == 'layout'){
                    if( is_tag() || is_category() || is_date() || is_year() || is_month() || is_author() ){
                        $_value = $this->get("layout_blog");
                        if(!empty($_value) && $_value !== 'inherit'){
                            $value = $_value;
                        }
        ...
        return apply_filters('raz/setting/get_setting_by_context', $value, $key, $context);
    }
