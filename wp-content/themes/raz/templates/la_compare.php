...
<div id="la_compare_table_wrapper">
    ...
    $lists = Raz_WooCommerce_Compare::get_data();

    $default_attr = raz_get_wc_attribute_for_compare();
    $tax_attr = raz_get_wc_attribute_taxonomies();
    $all_attr = array_merge($default_attr, $tax_attr);

    $attribute_allow = Raz()->settings()->get('compare_attribute', $default_attr);
    ...
    if (!empty($lists)) {
        foreach ($lists as $product_id) {
            ...
            $term['id'] = $product_id;
            $term['sku'] = ($_product->get_sku() ? $_product->get_sku() : esc_html__('N/A', 'raz'));
        ...
        <table class="la-compare-table">
            ...
            foreach( $attribute_allow as $attr_alow ){
                if(in_array($attr_alow, array('image','title','add-to-cart'))){
                    $label_list['info'] = esc_html_x('Product Info', 'front-view', 'raz');
                }
                else{
                ...
                foreach ($terms as $item) {
                    ...
                    elseif ($k == 'stock') {
                        ...
                        if ($item['availability'] && !empty($item['availability']['availability'])) {
                            ...
                            if ($item['stock']) {
                                $text_stock = esc_html__('In stock', 'raz');
                                $class_stock = 'in-stock';
                            } else {
                                $text_stock = esc_html__('Out of stock', 'raz');
        ...
        ?>
        <p class="text-center"><?php esc_html_e('No products were added to the list', 'raz') ?></p>
        <?php
    }
    ?>
</div>
