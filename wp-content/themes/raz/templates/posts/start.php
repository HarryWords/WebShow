...
$blog_item_space = Raz()->settings()->get('blog_item_space', 'default');

if($blog_item_space == 'zero'){
    $blog_item_space = 0;
}

$blog_design = Raz()->settings()->get('blog_design', 'grid_4');
$blog_columns = wp_parse_args( (array) Raz()->settings()->get('blog_post_column'), array('lg'=> 1,'md'=> 1,'sm'=> 1,'xs'=> 1, 'mb' => 1) );
$blog_masonry = ( Raz()->settings()->get('blog_masonry') == 'on' ) ? true : false;
...
if($layout == 'grid'){
    ...
    $css_classes[] = raz_render_grid_css_class_from_columns($blog_columns);
}
...
$thumbnail_size     = Raz_Helper::get_image_size_from_string(Raz()->settings()->get('blog_thumbnail_size', 'full'), 'full');
$excerpt_length     = Raz()->settings()->get('blog_excerpt_length');
$content_type       = (Raz()->settings()->get('blog_content_display', 'excerpt') == 'excerpt') ? 'excerpt' : 'full';
$show_thumbnail     = (Raz()->settings()->get('featured_images_blog') == 'on') ? true : false;
$height_mode        = 'original';
$thumb_custom_height= '';

raz_set_theme_loop_prop('is_main_loop', true, true);
raz_set_theme_loop_prop('loop_layout', $layout);
raz_set_theme_loop_prop('loop_style', $style);
raz_set_theme_loop_prop('title_tag', 'h2');
raz_set_theme_loop_prop('image_size', $thumbnail_size);
raz_set_theme_loop_prop('excerpt_length', $excerpt_length);
raz_set_theme_loop_prop('content_type', $content_type);
raz_set_theme_loop_prop('show_thumbnail', $show_thumbnail);
raz_set_theme_loop_prop('height_mode', $height_mode);
raz_set_theme_loop_prop('height', $thumb_custom_height);
raz_set_theme_loop_prop('responsive_column', $blog_columns);
