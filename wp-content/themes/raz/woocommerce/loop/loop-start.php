...
$active_shop_masonry = Raz()->settings()->get('active_shop_masonry', 'off');
$shop_masonry_column_type = Raz()->settings()->get('shop_masonry_column_type', 'default');
$woocommerce_shop_masonry_columns = Raz()->settings()->get('woocommerce_shop_masonry_columns');

$product_masonry_container_width = Raz()->settings()->get('product_masonry_container_width', 1170);
$product_masonry_item_width = Raz()->settings()->get('product_masonry_item_width', 270);
$product_masonry_item_height = Raz()->settings()->get('product_masonry_item_height', 450);
$woocommerce_shop_masonry_custom_columns = Raz()->settings()->get('woocommerce_shop_masonry_custom_columns');
$shop_masonry_item_setting = Raz()->settings()->get('shop_masonry_item_setting');


$woocommerce_pagination_type = Raz()->settings()->get('woocommerce_pagination_type', 'pagination');

$item_gap = Raz()->settings()->get('shop_item_space', 'default');

if($item_gap == 'zero'){
    $item_gap = 0;
}

$column_tmp = Raz()->settings()->get('woocommerce_shop_page_columns');

if($is_main_loop && raz_string_to_bool($active_shop_masonry)){
...
if($is_main_loop && raz_string_to_bool($active_shop_masonry) && $shop_masonry_column_type == 'custom'){
...
$design = Raz()->settings()->get("shop_catalog_grid_style", '1');
...
if($is_main_loop && raz_string_to_bool($active_shop_masonry)){
    raz_set_wc_loop_prop('prods_masonry', true);
    ...
    if( $shop_masonry_column_type != 'custom' ){
        ...
        $loopCssClass[] = raz_render_grid_css_class_from_columns($columns);
    }
    else{
        ...
        if(!empty($shop_masonry_item_setting) && is_array($shop_masonry_item_setting)){
            ...
            foreach($shop_masonry_item_setting as $k => $size){
                ...
                if(!empty($size['image_size'])){
                    $__new_item_sizes[$__k]['s'] = Raz_Helper::get_image_size_from_string($size['image_size']);
                }
                $__k++;
            }
        }
        raz_set_wc_loop_prop('item_sizes', $__new_item_sizes);
        $masonry_component_type[] = 'AdvancedMasonry';
    }
    raz_set_wc_loop_prop('image_size', Raz_Helper::get_image_size_from_string(Raz()->settings()->get('product_masonry_image_size', 'shop_catalog')));
    ?>
<?php
}
else{
