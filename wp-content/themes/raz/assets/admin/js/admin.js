    ...
    $(function () {
        ...
        if(typeof pagenow !== "undefined" && pagenow === "widgets"){
            ...
            $create_box.find( '.la_pb_create_widget_area' ).click( function( event ) {
                ...
                $.ajax( {
                    type: "POST",
                    url: raz_sidebar_options.ajaxurl,
                    data:
                        {
                            action : 'raz_core_action',
                            router : 'add_sidebar',
                            admin_load_nonce : raz_sidebar_options.admin_load_nonce,
            ...
            $la_pb_sidebars.each( function() {
                ...
                $(this).closest('.widgets-holder-wrap').find('.sidebar-name h2, .sidebar-name h3').before( '<a href="#" class="la_pb_widget_area_remove">' + raz_sidebar_options.delete_string + '</a>' );
                ...
                $( '.la_pb_widget_area_remove' ).click( function( event ) {
                    ...
                    if(confirm(raz_sidebar_options.confirm_delete_string)){
                        $.ajax( {
                            type: "POST",
                            url: raz_sidebar_options.ajaxurl,
                            data:
                                {
                                    ...
                                    admin_load_nonce : raz_sidebar_options.admin_load_nonce,
                                    widget_area_name : $this_el.closest( '.widgets-holder-wrap' ).find( 'div[id^=raz_widget_area_]' ).attr( 'id' )
                                },
                            success: function( data ){
                                if(data.success){
